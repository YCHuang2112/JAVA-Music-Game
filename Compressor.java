...
public class Compressor extends /*JPanel*/PlugIn implements ActionListener
{
	...
	Compressor(String fileName,int order)
	{
		super(fileName,"compressor",order);
		...
		enterButton=new JButton("compress");
				...
				for(int i=0;i<numberOfChannel;i++)
					if(Math.abs(buffer_Read[i][s]*volumeSend)>threshold)
						if(buffer_Read[i][s]>0)
									buffer_Write[i][s] = gain*((buffer_Read[i][s]*volumeSend-threshold)*ratio+threshold)
								+buffer_Read[i][s]*(1-volumeSend);
						else
							buffer_Write[i][s] = gain*((buffer_Read[i][s]*volumeSend+threshold)*ratio-threshold)
								+buffer_Read[i][s]*(1-volumeSend);
					else
