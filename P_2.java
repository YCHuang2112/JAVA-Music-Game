...
public class P_2 extends JPanel  implements KeyListener,MouseListener, MouseMotionListener 
{
	...
	private int speed_controller = User_Settings.SPEED_CONTROL_VALUE;//for speed controlling
	private int SPEED_HEIGHT_DEVIDENT = 800;   // devide gameboard y_axis to ( SPEED_HEIGHT_DEVIDENT ) compartment
	private int hit_time_error_tollerent = User_Settings.HIT_ERROR_TOLERENT; // y-axis value
	...
	*/
	...
	//begin:
	private int converge_mode = User_Settings.CIRCLE_CONVERGE_MODE;		//mode 1: fasten outter radius  to different converge speed; mode 2 : outter radius propotional to the converge speed
	//mode 1:
		...
		for(trackNumber = 0; trackNumber < tracks.length; trackNumber++)
		{
			...
			{ 
                ...
                if (message instanceof ShortMessage) 
				{
					...
					int message_com = 0;
					if ( (message_com = sm.getCommand()) == NOTE_ON && (i%User_Settings.INSTANCE_SPARSITY==0)) 
					{
						tick = event.getTick();
...
//System.out.printf("x = %d, x_tran = %d, y = %d, y_tran = %d, dist = %f, rad*rad = %f%n", x, x_tran, y, y_tran, dist, rad*rad);
	...
	public boolean get_is_hit(){return this.is_hit;}
}
...
//System.out.printf("time in paint:%d %n ", time_duration);
		//
							...
							if(time_duration >= (long)pre_time)
							{
								...
								g.drawString("Time:                            ", getWidth()-150,25);
								g.drawString("Time: " + (time_duration-(long)pre_time)/divide_to_S + " s", getWidth()-150,25);
							}
							else
							{
							...
							g.drawString("hits : " + hits, getWidth()-150,50);
							g.drawString("miss : " + error_accumulation, getWidth()-150,75);
							if(iscombo){g.drawString("combo! now", getWidth()-150,110);}
	...
	private void clean_out_bound()
	{
		...
		if(k > 0)
		{
			for(int i = 0; i < k; i++)
			{
				...
				if(!converge_circles_linkedlist.get(j).get_is_hit()){
					error_accumulation += 1;
				}
